...
class CNNClassifier(nn.Module):
    def __init__(self, num_classes):
        ...
        self.layer1 = nn.Sequential(ResSEBlock(2,   64, kernel_size=3, padding=1),
                                    nn.MaxPool1d(2))  # → (B,64,L/2)
        self.layer2 = nn.Sequential(ResSEBlock(64, 128, kernel_size=3, padding=1),
                                    nn.MaxPool1d(2))  # → (B,128,L/4)
        self.layer3 = nn.Sequential(ResSEBlock(128,256, kernel_size=3, padding=1),
                                    nn.MaxPool1d(2))  # → (B,256,L/8)

        self.transformer = TimeTransformer(dim=256)  
